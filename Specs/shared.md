...
- **El cache de TanStack Query se persiste en IndexedDB** (`persistencia-query.ts` + `use-cache-persistente.ts`). Es lo que deja capturar sin señal: sin las lecturas rehidratadas la pantalla de captura no se pinta. Usa `dehydrate`/`hydrate` del core, **sin dependencia nueva**. Lo que entra al blob es una **allowlist explícita** (`src/app/claves-persistibles.ts`): solo lo que la captura sin señal necesita. `planilla` (salario y moneda), `perfil` y los rollups de admin quedan afuera — el blob vive en el disco del dispositivo 7 días, sin cifrar. Tres condiciones que no se pueden aflojar: `gcTime` ≥ la vigencia de lo persistido (con el default de 5 min lo rehidratado se recolecta al montar), nada se pinta hasta rehidratar (montar antes dispara las queries contra la red), y **ninguna de las dos promesas puede quedar sin `catch`**: un IndexedDB que rechaza al restaurar dejaba la app en blanco para siempre, porque el router ni llega a montarse.
