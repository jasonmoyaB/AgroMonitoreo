...
- `src/app/router.tsx` — React Router data router, only route wiring. `/login` and `/registro` are public; everything else (`/supervisor`, `/supervisor/dashboard`, `/supervisor/trabajadores`, `/supervisor/asistencia`, `/captura/*`) sits behind `AuthGuard`.
