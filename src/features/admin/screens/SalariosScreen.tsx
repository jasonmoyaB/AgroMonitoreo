...
import { useActualizarSalario } from '../hooks/use-actualizar-salario'
    ...
    <main className="h-dvh overflow-hidden p-3 sm:p-4">
      <div className="flex h-full min-w-0 flex-col gap-3 md:flex-row md:gap-4">
        ...
        <section className="min-h-0 min-w-0 flex-1 overflow-y-auto overscroll-contain">
          ...
          <SalariosTable trabajadores={trabajadores.trabajadores} isLoading={trabajadores.isLoading} onCambiarSalario={actualizarSalario.mutate} />
        </section>
      </div>
    </main>
  )
